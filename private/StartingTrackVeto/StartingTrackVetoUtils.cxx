	...
	I3GeometryConstPtr geo, I3VectorOMKeyPtr badDOMs )
{
	...
	//DOM expected charge
        ...
        for ( unsigned int i = 0; i < timeEdges.size(); i++ )
        {
            ...
            if ( sumQuantilesPerTime[i] > percent * maxQuantilePerTime ){ gtThreshIdxs.push_back(i); }
        }
	...
	double coincObsPsTmp=0;
        ...
        for ( unsigned int  i = 0; i< qs.size(); i++ )
        {
            ...
            if ( ts[i] > minTime && ts[i] < maxTime ){ coincObsQsTmp+=qs[i]; coincObsPsTmp+=1; }
        }
        log_debug("coinc obs q= %f",coincObsQsTmp);
	log_debug("coinc obs p= %f",coincObsPsTmp);
        //If for some reason you encounter a nan value, indicating something bad, record 0 and keep moving
        if ( std::isnan( quantileSum ) ) { coincObsQsList.push_back( 0 ); coincObsPsList.push_back( 0 ); }
        else { coincObsQsList.push_back( coincObsQsTmp ); coincObsPsList.push_back( coincObsPsTmp ); }
        index++;
    }
    //Book calculation pieces to the frame
    std::string namePrefix ("TrackHits_"+fitName+"_"+pulsesName);
    std::string nameSufixes[6] = { "coincObsQsList", "coincObsPsList", 
				   "expectedQsList", "expectedBgsList",
				   "closestApDists", "keyToIndexMap" };
    log_debug("Names set");
